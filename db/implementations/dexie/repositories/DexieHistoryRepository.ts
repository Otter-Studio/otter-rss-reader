/**
 * Dexie ReadHistory 仓库实现
 */

import { IHistoryRepository } from '../../../abstractions';
import { IReadHistory, IPaginationParams, IPaginatedResult } from '../../../models';

export class DexieHistoryRepository implements IHistoryRepository {
  constructor(private db: any) {
    if (!db) {
      console.warn('[DexieHistoryRepository] Database instance not provided');
    }
  }

  async add(history: IReadHistory): Promise<IReadHistory> {
    throw new Error('[DexieHistoryRepository] Not implemented - History repository is under development for Web platform');
  }
